...
--QUERY--
--1
SELECT map_code, AVG( EXTRACT(HOUR FROM (endtime - starttime)) * 3600 + EXTRACT(MINUTE FROM (endtime - starttime)) * 60 + EXTRACT(SECOND FROM (endtime - starttime)) ) AS avg_seconds FROM Game GROUP BY map_code;
--3
...
--4
SELECT ux.response_id, p.alias, ux.item_id, ux.response_value 
FROM UX_responseItem ux JOIN Game g ON ux.response_id = g.player_id JOIN PLAYER p ON g.player_id = p.player_id 
WHERE (g.endtime - g.starttime) > ( SELECT AVG(endtime - starttime) FROM Game ) ORDER BY ux.response_id, ux.item_id; 
--5
...
--7 
WITH shortest_players AS ( SELECT g.player_id, m.episode_id, EXTRACT(EPOCH FROM (g.endtime - g.starttime)) as duration, p.alias FROM Game g JOIN Map m ON g.map_code = m.map_code JOIN Player p ON g.player_id = p.player_id WHERE EXTRACT(EPOCH FROM (g.endtime - g.starttime)) = ( SELECT MIN(EXTRACT(EPOCH FROM (g2.endtime - g2.starttime))) FROM Game g2 JOIN Map m2 ON g2.map_code = m2.map_code WHERE m2.episode_id = m.episode_id ) ) SELECT sp.episode_id, sp.player_id, sp.alias, sp.duration, ROUND(AVG( CASE WHEN ux.response_value = 'Strongly Disagree' THEN 1 WHEN ux.response_value = 'Disagree' THEN 2 WHEN ux.response_value = 'Somewhat Disagree' THEN 3 WHEN ux.response_value = 'Neither Agree nor Disagree' THEN 4 WHEN ux.response_value = 'Somewhat Agree' THEN 5 WHEN ux.response_value = 'Agree' THEN 6 WHEN ux.response_value = 'Strongly Agree' THEN 7 ELSE 4 END )::numeric, 2) as average_ux_score FROM shortest_players sp JOIN Ux_responseItem ux ON sp.player_id = ux.response_id GROUP BY sp.episode_id, sp.player_id, sp.alias, sp.duration ORDER BY sp.episode_id, sp.player_id;
